 ...
 * 2001-10-24 Christian d'Heureuse(chdh): VBasic version created.<br />
                        ...
                        *   No error("unknown command") is generated any more, if a HTML comment starts with "${".<br />
                            ...
                            public $maxNestingLevel = 50;            // maximum number of block nestings
                            public $maxInclTemplateSize = 1000000;   // maximum length of template string when including subtemplates
                            public $template;                        // Template file data
                            public $varTab;                          // variables table, array index is variable no
                            /* Fields:
                             * varName                       // variable name
                             * varValue                      // variable value */
                            public $varTabCnt;                       // no of entries used in VarTab
                            public $varNameToNoMap;                  // maps variable names to variable numbers
                            public $varRefTab;                       // variable references table
                            ...
                            public $varRefTabCnt;                    // no of entries used in VarRefTab
                            public $blockTab;                        // Blocks table, array index is block no
                            ...
                            public $blockTabCnt;                     // no of entries used in BlockTab
                            public $blockNameToNoMap;                // maps block names to block numbers
                            public $openBlocksTab;
                            ...
                            public $blockInstTab;                    // block instances table
                            ...
                            public $blockInstTabCnt;                 // no of entries used in BlockInstTab

                            public $currentNestingLevel;             // Current block nesting level during parsing.
                            public $templateValid;                   // true if a valid template is prepared
                            public $outputMode;                      // 0 = to PHP output stream, 1 = to file, 2 = to string
                            public $outputFileHandle;                // file handle during writing of output file
                            public $outputError;                     // true when an output error occurred
                            public $outputString;                    // string buffer for the generated HTML page
                             ...
                             * @access private
                             */
                            public function loadSubtemplate ($subtemplateName, &$s) {
                             ...
                             * @access private
                             */
                            public function beginMainBlock() {
                                $blockNo = 0;
                             ...
                             * @access private
                             */
                            public function parseTemplateCommands() {
                                $p = 0;
                                while (true) {
                             ...
                             * @access private
                             */
                            public function processTemplateCommand ($cmdL, $cmdTPosBegin, $cmdTPosEnd, &$resumeFromStart) {
                             ...
                             * @access private
                             */
                            public function processBeginBlockCmd ($parms, $cmdTPosBegin, $cmdTPosEnd) {
                                $p = 0;
                             ...
                             * @access private
                             */
                            public function processEndBlockCmd ($parms, $cmdTPosBegin, $cmdTPosEnd) {
                                $p = 0;
                                if (!$this->parseWord($parms,$p,$blockName)) {
                             ...
                             * @access private
                             */
                            public function registerBlock($blockName, &$blockNo) {
                             ...
                             * @access private
                             */
                            public function checkBlockDefinitionsComplete() {
                             ...
                             * @access private
                             */
                            public function processIncludeCmd($parms, $cmdTPosBegin, $cmdTPosEnd) {
                                $p = 0;
                             ...
                             * @access private
                             */
                            public function insertSubtemplate($subtemplateName, $tPos1, $tPos2) {
                            ...
                            /**
                             * Parses variable references within the template in the format "${
                             /**
                             *
                             */
                            VarName}".
     ...
     * @access private
     */
    public function registerVariableReference ($varName, $tPosBegin, $tPosEnd) {
     ...
     * @access private
     */
    public function registerVariable ($varName, &$varNo) {
     ...
     * @access private
     */
    public function associateVariablesWithBlocks() {
        $varRefNo = 0;
        $activeBlockNo = 0;
        $nextBlockNo = 1;
 ...
 /* --- build up (template variables and blocks) ---------------------------------------------------------------------- */
     ...
     * the characters &lt;, &gt;, &amp;, ' and " of variableValue are
                     ...
                     * @access private
                     */
                    public function addBlockByNo($blockNo) {
                        $btr =& $this->blockTab[$blockNo];
                     ...
                     * @access private
                     */
                    public function registerBlockInstance(&$blockInstNo) {
                     ...
                     * @access private
                     */
                    public function writeBlockInstances($blockNo, $parentInstLevel) {
                        $btr =& $this->blockTab[$blockNo];
                     ...
                     * @access private
                     */
                    public function writeBlockInstance($blockInstNo) {
                        $bitr =& $this->blockInstTab[$blockInstNo];
                     ...
                     * @access private
                     */
                    public function lookupVariableName($varName, &$varNo) {
                     ...
                     * @access private
                     */
                    public function lookupBlockName($blockName, &$blockNo) {
                     ...
                     * @access private
                     */
                    public function readFileIntoString($fileName, &$s) {
                     ...
                     */
                    public function parseWord($s, &$p, &$w) {
                        $sLen = strlen($s);
                     ...
                     * @return boolean  true on success, false on error.
                     */
                    public function parseQuotedString($s, &$p, &$w) {
                        $sLen = strlen($s);
                        while ($p < $sLen && ord($s{$p}) <= 32) $p++;
                        if ($p >= $sLen) return false;
                     ...
                     * @return boolean  true on success, false on error.
                     */
                    public function parseWordOrQuotedString($s, &$p, &$w) {
                        $sLen = strlen($s);
                        while ($p < $sLen && ord($s{$p}) <= 32) $p++;
                        if ($p >= $sLen) return false;
                        if (substr($s, $p, 1) == '"')
                     ...
                     * @access private
                     */
                    public function combineFileSystemPath($path1, $path2) {
                     ...
                     * @access private
                     */
                    public function triggerError($msg) {
                        trigger_error("MiniTemplator error: $msg", E_USER_ERROR); }

                    /**
                     * @access private
                     */
                    public function programLogicError($errorId) {
